...
using Shopdrawing.Framework.Runtime;
        ...
        public static List<IPropertyGridItem> GetMethodItems(object instance)
        {
            ...
            if (instance != null)
            {
                foreach (var methodInfo in instance.GetType().GetMethods())
                {
                    ...
                    //
                    object[] objs = methodInfo.GetCustomAttributes(typeof(CommandMethodAttribute), true);
                    foreach (object obj in objs)
                    {
                        if (obj is CommandMethodAttribute)
                        {
                            var att = obj as CommandMethodAttribute;
                            methodItems.Add(new MethodItem(att.GlobalName, methodInfo, instance));
                        }
                    }
                }
            }
