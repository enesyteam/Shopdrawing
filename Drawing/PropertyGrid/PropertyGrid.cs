...
namespace DynamicGeometry
{
    public partial class PropertyGrid : StackPanel
    {
        ...
        public virtual void ShowProperties(object selection)
        {
            try
            {
                this.Show(selection, Drawing.ActionManager);

                //pg.SelectedObject = selection; // CC
            }
            catch (Exception ex)
            {
        ...
        protected virtual void AddHeader()
        {
            ...
            if (string.IsNullOrEmpty(title))
            {
                ...
                // CC
                //this.ToolTip = GetDescription(Selection);
        ...
        // CC
        protected static string GetDescription(object editableObject)
        {
            var result = editableObject.ToString();
            var type = editableObject.GetType();
            var attribute = type.GetAttribute<PropertyGridDescriptionAttribute>();
            if (attribute != null)
            {
        ...
        {
            return new MethodCallerButton() { OperationDescription = m, Target = obj, Description = "Test description" };
        }
