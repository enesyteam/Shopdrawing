...
namespace Shopdrawing.Core.Utilities.Csv
{
    /// <summary>
    ...
    public partial class CsvReader : IDataReader, IEnumerable<string[]>
    {
        ...
        public CsvReader(TextReader reader, bool hasHeaders, char delimiter, char quote, char escape, char comment, bool trimSpaces, int bufferSize)
        {
            ...
            if (bufferSize <= 0)
                throw new ArgumentOutOfRangeException("bufferSize", bufferSize, Properties.Resources.BufferSizeTooSmall);
        ...
        public string this[string field]
        {
            get
            {
                ...
                if (!_hasHeaders)
                    throw new InvalidOperationException(Properties.Resources.NoHeaders);

                int index = GetFieldIndex(field);

                if (index < 0)
                    throw new ArgumentException(string.Format(CultureInfo.InvariantCulture, Properties.Resources.FieldHeaderNotFound, field), "field");
        ...
        public void CopyCurrentRecordTo(string[] array, int index)
        {
            ...
            if (array.Length - index < _fieldCount)
                throw new ArgumentException(Properties.Resources.NotEnoughSpaceInArray, "array");
        ...
        public virtual void MoveTo(long record)
        {
            ...
            if (record < _currentRecordIndex)
                throw new InvalidOperationException(Properties.Resources.CannotMovePreviousRecordInForwardOnly);
            ...
            if (offset > 0)
                do
                    if (!ReadNextRecord())
                        throw new EndOfStreamException(string.Format(CultureInfo.InvariantCulture, Properties.Resources.CannotReadRecordAtIndex, _currentRecordIndex - offset));
                while (--offset > 0);
        }
            ...
            switch (DefaultParseErrorAction)
            {
                ...
                case ParseErrorAction.RaiseEvent:
                    ...
                    switch (e.Action)
                    {
                        ...
                        case ParseErrorAction.RaiseEvent:
                            throw new InvalidOperationException(string.Format(CultureInfo.InvariantCulture, Properties.Resources.ParseErrorActionInvalidInsideParseErrorEvent, e.Action), e.Error);
                        ...
                        default:
                            throw new NotSupportedException(string.Format(CultureInfo.InvariantCulture, Properties.Resources.ParseErrorActionNotSupported, e.Action), e.Error);
                    }
                    break;
                ...
                default:
                    throw new NotSupportedException(string.Format(CultureInfo.InvariantCulture, Properties.Resources.ParseErrorActionNotSupported, DefaultParseErrorAction), error);
            }
        }
        ...
        private string HandleMissingField(string value, int fieldIndex, ref int currentPosition)
        {
            if (fieldIndex < 0 || fieldIndex >= _fieldCount)
                throw new ArgumentOutOfRangeException("fieldIndex", fieldIndex, string.Format(CultureInfo.InvariantCulture, Properties.Resources.FieldIndexOutOfRange, fieldIndex));
            ...
            switch (MissingFieldAction)
            {
                ...
                default:
                    throw new NotSupportedException(string.Format(CultureInfo.InvariantCulture, Properties.Resources.MissingFieldActionNotSupported, MissingFieldAction));
            }
        }
        ...
        private void ValidateDataReader(DataReaderValidations validations)
        {
            ...
            if ((validations & DataReaderValidations.IsNotClosed) != 0 && _isDisposed)
                throw new InvalidOperationException(Properties.Resources.ReaderClosed);
        }
        ...
        Type IDataRecord.GetFieldType(int i)
        {
            ...
            if (i < 0 || i >= _fieldCount)
                throw new ArgumentOutOfRangeException("i", i, string.Format(CultureInfo.InvariantCulture, Properties.Resources.FieldIndexOutOfRange, i));
        ...
        int IDataRecord.GetOrdinal(string name)
        {
            ...
            if (!_fieldHeaderIndexes.TryGetValue(name, out index))
                throw new ArgumentException(string.Format(CultureInfo.InvariantCulture, Properties.Resources.FieldHeaderNotFound, name), "name");
