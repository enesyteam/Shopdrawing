...
namespace Microsoft.Expression.DesignSurface.UserInterface.PropertyInspector
{
  ...
  {
    public static readonly DependencyProperty WrappedValueProperty = DependencyProperty.Register("WrappedValue", typeof (object), typeof (TextChoiceEditorWrapper), (PropertyMetadata) new FrameworkPropertyMetadata((object) null, FrameworkPropertyMetadataOptions.BindsTwoWayByDefault, new PropertyChangedCallback(TextChoiceEditorWrapper.WrappedValueChanged), (CoerceValueCallback) null, false, UpdateSourceTrigger.PropertyChanged));
    ...
    public UnitType UnitType
    {
      ...
      set
      {
        this.SetValue(TextChoiceEditorWrapper.UnitTypeProperty, (object) value);
      }
    }
    ...
    {
      this.choiceEditorValueDescriptor.AddValueChanged((object) this.TextChoiceEditor, new EventHandler(this.ChoiceEditorValueChanged));
    }

    private void TextChoiceEditorWrapper_Unloaded(object sender, RoutedEventArgs e)
    {
      this.choiceEditorValueDescriptor.RemoveValueChanged((object) this.TextChoiceEditor, new EventHandler(this.ChoiceEditorValueChanged));
    }
    ...
    private double EnforceHardMaximumAndMinimum(double size)
    {
      ...
      {
        SceneNodeProperty sceneNodeProperty = nodePropertyValue.ParentProperty as SceneNodeProperty;
        if (sceneNodeProperty != null && sceneNodeProperty.ValueEditorParameters != null)
        {
          ...
          {
            if (numberRangesAttribute.HardMaximum.HasValue && numberRangesAttribute.HardMaximum.HasValue)
              size = Math.Min(numberRangesAttribute.HardMaximum.Value, size);
            if (numberRangesAttribute.HardMinimum.HasValue && numberRangesAttribute.HardMinimum.HasValue)
              size = Math.Max(numberRangesAttribute.HardMinimum.Value, size);
          }
        }
      }
      return size;
    ...
    private static void WrappedValueChanged(DependencyObject d, DependencyPropertyChangedEventArgs e)
    {
      ...
      try
      {
        ...
        if (newValue != null && newValue != MixedProperty.Mixed && newValue is double)
          ...
          choiceEditorWrapper.TextChoiceEditor.Value = (object) null;
      }
      finally
      {
        choiceEditorWrapper.isUpdatingValue = false;
    ...
    public void InitializeComponent()
    {
      ...
      Application.LoadComponent((object) this, new Uri("/Microsoft.Expression.DesignSurface;component/userinterface/propertyinspector/categoryeditors/text/textchoiceeditorwrapper.xaml", UriKind.Relative));
    }
