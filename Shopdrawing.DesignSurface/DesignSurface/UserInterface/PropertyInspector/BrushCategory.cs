...
namespace Microsoft.Expression.DesignSurface.UserInterface.PropertyInspector
{
  public sealed class BrushCategory : SceneNodeCategory
  {
    ...
    private void UpdateSelectionSet()
    {
      ...
      this.forcedToSelectOpacityMask = this.brushCollectionView.Count == 1 && string.Compare(referenceProperty.PropertyName, UIElement.OpacityMaskProperty.Name, StringComparison.Ordinal) == 0;
    }

    private void OnBasicPropertiesChanged(object sender, NotifyCollectionChangedEventArgs e)
    {
      this.brushCollectionView.MoveCurrentTo((object) null);
    }
    ...
    public override void OnSelectionChanged(SceneNode[] selectedObjects)
    {
      ...
      if (this.designerContext != null && this.designerContext.ActiveSceneViewModel != null)
      {
        ...
        {
          foreach (PropertyReferenceProperty referenceProperty2 in (Collection<PropertyEntry>) this.BasicProperties)
          {
            if (string.Compare(referenceProperty2.PropertyName, primarySelection.ShortPath, StringComparison.Ordinal) == 0)
            {
    ...
    private class PropertyComparer : System.Collections.Generic.Comparer<PropertyEntry>
    {
      ...
      {
        if (string.Compare(first.PropertyName, UIElement.OpacityMaskProperty.Name, StringComparison.Ordinal) == 0)
          return 1;
        if (string.Compare(second.PropertyName, UIElement.OpacityMaskProperty.Name, StringComparison.Ordinal) == 0)
          return -1;
        return string.Compare(first.PropertyName, second.PropertyName, StringComparison.Ordinal);
      }
    }
  }
}
