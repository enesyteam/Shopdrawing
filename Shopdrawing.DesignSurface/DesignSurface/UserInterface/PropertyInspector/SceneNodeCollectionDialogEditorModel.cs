...
namespace Microsoft.Expression.DesignSurface.UserInterface.PropertyInspector
{
  public class SceneNodeCollectionDialogEditorModel : CollectionDialogEditorModel
  {
    ...
    private SceneNodeObjectSet SceneNodeObjectSet
    {
      get
      {
        if (this.sceneNodeObjectSet == null && this.Host != null && (this.Host.PropertyValue != null && this.Host.PropertyValue.ParentProperty != null))
        {
          this.sceneNodeObjectSet = ((SceneNodeProperty) this.Host.PropertyValue.ParentProperty).SceneNodeObjectSet;
    ...
    public override object CreateType(Type type)
    {
      if (type == (Type) null)
      {
        SceneNodeProperty sceneNodeProperty = this.Host.PropertyValue.ParentProperty as SceneNodeProperty;
        if (sceneNodeProperty != null)
        {
          SceneViewModel viewModel = sceneNodeProperty.SceneNodeObjectSet.ViewModel;
          ReferenceStep step = (ReferenceStep) IndexedClrPropertyReferenceStep.GetReferenceStep((ITypeResolver) viewModel.ProjectContext, sceneNodeProperty.PropertyType, 0);
          Type propertyType = sceneNodeProperty.Reference.Append(step).ValueType;
          Type genericCollectionType = CollectionAdapterDescription.GetGenericCollectionType(sceneNodeProperty.PropertyType);
    ...
    {
      return ((PropertyReferenceProperty) this.Host.PropertyValue.ParentProperty).GetAttributes<NewItemTypesAttribute>(true);
    }
