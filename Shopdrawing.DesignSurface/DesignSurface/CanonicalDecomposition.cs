...
// Type: Microsoft.Expression.DesignSurface.CanonicalDecomposition
...
{
  public class CanonicalDecomposition : ICloneable
  {
    ...
    public void ApplySkewScale(Vector basisX, Vector basisY, Point origin, Point fixedPoint, Vector newBasisX, Vector newBasisY)
    {
      ...
      if (matrix4.M22 == 0.0 && matrix4.M21 != 0.0 || matrix4.M11 == 0.0 && matrix4.M12 != 0.0)
        throw new InvalidOperationException(ExceptionStringTable.CannotRealizeThisTransformViaSkewScale);
      double x = 0.0;
      double y = 0.0;
      if (matrix4.M22 != 0.0)
    ...
    private void ConvertGenericTransform(Matrix transform)
    {
      ...
      Vector vector3 = vector1 * transform;
      Vector vector4 = vector2 * transform;
      Point point2 = point1 * transform;
      this.Translation = new Vector(point2.X, point2.Y);
      this.Scale = new Vector(vector3.Length, vector4.Length);
      bool flag1 = vector3.LengthSquared > CanonicalDecomposition.tolerance;
      bool flag2 = vector4.LengthSquared > CanonicalDecomposition.tolerance;
      double num1 = 0.0;
      double num2 = 0.0;
      if (flag1)
        num1 = this.GetAngle(new Vector(1.0, 0.0), vector3);
      if (flag2)
        num2 = this.GetAngle(new Vector(0.0, 1.0), vector4);
      if (flag1 && flag2 && Math.Abs(Vector.CrossProduct(vector3, vector4)) <= CanonicalDecomposition.tolerance)
      {
        Vector v2 = Math.Abs(num1) <= Math.Abs(num2) ? vector3 : vector4;
      ...
      else
      {
        ...
        {
          Vector vector5 = vector3 / this.Scale.X;
          this.ScaleY = (vector4 - vector5 * (vector5.X * vector4.X + vector5.Y * vector4.Y)).Length;
          this.RotationAngle = num1;
          if (!flag2)
            return;
          double angle = this.GetAngle(vector3, vector4);
          if (angle < 0.0)
          {
            this.ScaleY = -this.ScaleY;
        ...
        {
          Vector vector5 = vector4 / this.Scale.Y;
          this.ScaleX = (vector3 - vector5 * (vector5.X * vector3.X + vector5.Y * vector3.Y)).Length;
          this.RotationAngle = num2;
          if (!flag1)
            return;
          double angle = this.GetAngle(vector4, vector3);
          if (angle > 0.0)
          {
            this.ScaleX = -this.ScaleX;
