  ...
  public sealed class Vector3DEditor : Grid, INotifyPropertyChanged, IComponentConnector
  {
    ...
    private void OnDataContextChangedRebuild(object sender, DependencyPropertyChangedEventArgs e)
    {
      if (this.editingProperty != null)
      {
        ...
        this.editingProperty.remove_PropertyChanged(new PropertyChangedEventHandler(this.OnEditingPropertyPropertyChanged));
      ...
      if (this.editingProperty != null)
      {
        this.editingProperty.add_PropertyChanged(new PropertyChangedEventHandler(this.OnEditingPropertyPropertyChanged));
        this.editingProperty.PropertyReferenceChanged += new Microsoft.Expression.DesignSurface.Documents.PropertyReferenceChangedEventHandler(this.OnVector3DChanged);
      }
      this.Rebuild();
    }
    ...
    {
      if (typeof (Vector3D).IsAssignableFrom(((PropertyEntry) this.editingProperty).get_PropertyType()))
        this.editingProperty.SetValue((object) new Vector3D(this.x, this.y, this.z));
      else if (typeof (Point3D).IsAssignableFrom(((PropertyEntry) this.editingProperty).get_PropertyType()))
    ...
    public void InitializeComponent()
    {
      ...
      Application.LoadComponent(this, new Uri("/Microsoft.Expression.DesignSurface;component/userinterface/propertyinspector/complexvalueeditors/vector3deditor.xaml", UriKind.Relative));
    }
