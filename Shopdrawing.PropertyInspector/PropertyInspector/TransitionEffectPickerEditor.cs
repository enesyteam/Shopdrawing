  ...
  public sealed class TransitionEffectPickerEditor : Grid, INotifyPropertyChanged, IComponentConnector
  {
    ...
    public TransitionEffectInfo CurrentTransitionEffect
    {
      get
      {
        ...
        {
          object obj = this.EditingProperty.get_PropertyValue().get_Value();
      ...
      set
      {
        if (this.EditingProperty.get_PropertyValue() == null || this.EditingProperty.SceneNodeObjectSet.RepresentativeSceneNode == null)
          return;
        using (SceneEditTransaction editTransaction = this.EditingProperty.SceneNodeObjectSet.ViewModel.CreateEditTransaction(StringTable.ChangeTransitionEffectUndoUnit, false))
        {
          if (value.Asset == null)
          {
            this.EditingProperty.get_PropertyValue().set_Value(null);
          }
          else
          {
            ...
            if (((TypeAsset) value.Asset).EnsureTypeReferenced(projectContext))
            {
              ...
              if (defaultConstructor != (ConstructorInfo) null)
                this.EditingProperty.get_PropertyValue().set_Value(defaultConstructor.Invoke((object[]) null));
            }
    ...
    public IEnumerable<PropertyEntry> SubProperties
    {
      get
      {
        if (this.editingProperty == null || this.editingProperty.get_PropertyValue().get_SubProperties().get_Count() <= 0)
          return (IEnumerable<PropertyEntry>) null;
        DependencyPropertyValueSource propertyValueSource = this.editingProperty.get_PropertyValue().get_Source() as DependencyPropertyValueSource;
        if (propertyValueSource == null || !propertyValueSource.get_IsResource())
          return (IEnumerable<PropertyEntry>) this.editingProperty.get_PropertyValue().get_SubProperties();
        foreach (PropertyBase propertyBase in this.subproperties)
          propertyBase.OnRemoveFromCategory();
        this.subproperties.Clear();
        using (IEnumerator<PropertyEntry> enumerator = this.editingProperty.get_PropertyValue().get_SubProperties().GetEnumerator())
        {
          while (((IEnumerator) enumerator).MoveNext())
            this.subproperties.Add(this.editingProperty.SceneNodeObjectSet.CreateSceneNodeProperty(this.editingProperty.Reference.Append(((PropertyReferenceProperty) enumerator.Current).Reference), (AttributeCollection) null));
        }
    ...
    private void Rebuild()
    {
      ...
      this.transitionEffectsView = CollectionViewSource.GetDefaultView(this.transitionEffects);
    ...
    public void InitializeComponent()
    {
      ...
      Application.LoadComponent(this, new Uri("/Microsoft.Expression.DesignSurface;component/userinterface/propertyinspector/complexvalueeditors/transitioneffectpickereditor.xaml", UriKind.Relative));
    }
    ...
    private class AlphabeticalTransitionEffectComparer : System.Collections.Generic.Comparer<TransitionEffectInfo>
    {
