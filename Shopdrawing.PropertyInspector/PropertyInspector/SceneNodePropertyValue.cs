...
namespace Microsoft.Expression.DesignSurface.UserInterface.PropertyInspector
{
  public class SceneNodePropertyValue : PropertyReferencePropertyValue, IDragEditNotifier
  {
    ...
    public override bool IsCollection
    {
      get
      {
        return typeof (IList).IsAssignableFrom(((PropertyEntry) this.property).get_PropertyType());
      }
    }
    ...
    public bool CanBeAuto
    {
      get
      {
        if (this.property.ValueEditorParameters != null)
        {
          ...
          if (numberRangesAttribute != null && (numberRangesAttribute.get_CanBeAuto().HasValue && numberRangesAttribute.get_CanBeAuto().Value))
    ...
    {
      bool flag = false;
      if (this.isEditorDragging && this.IsAutoDraggableProperty && (this.get_Value() is double && double.IsNaN((double) this.get_Value())))
      {
        flag = this.TrySetAutoSizeProperty(LayoutPathNode.CapacityProperty, value, (IPropertyId) null, null, (Func<Thickness, Size, double, Thickness>) null);
        if (!flag)
    ...
    private bool TrySetAutoSizeProperty(IPropertyId layoutProperty, object value, IPropertyId alignmentProperty, object autoAlignmentValue, Func<Thickness, Size, double, Thickness> adjustMarginFunction)
    {
      if (this.property.Reference.FirstStep.Equals((object) layoutProperty))
      {
        ...
        {
          (object) ((PropertyEntry) this.property).get_PropertyName()
        });
