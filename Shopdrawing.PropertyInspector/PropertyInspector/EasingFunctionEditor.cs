...
namespace Microsoft.Expression.DesignSurface.UserInterface.PropertyInspector
{
  ...
  public sealed class EasingFunctionEditor : Grid, INotifyPropertyChanged, IComponentConnector
  {
    ...
    public IEnumerable<PropertyEntry> SubProperties
    {
      get
      {
        if (this.easingFunctionProperty == null || this.IsEasingFunctionNinched || this.easingFunctionProperty.get_PropertyValue().get_SubProperties().get_Count() <= 0)
          return (IEnumerable<PropertyEntry>) null;
        DependencyPropertyValueSource propertyValueSource = this.easingFunctionProperty.get_PropertyValue().get_Source() as DependencyPropertyValueSource;
        if (propertyValueSource == null || !propertyValueSource.get_IsResource())
          return (IEnumerable<PropertyEntry>) this.easingFunctionProperty.get_PropertyValue().get_SubProperties();
        foreach (PropertyBase propertyBase in this.subproperties)
          propertyBase.OnRemoveFromCategory();
        this.subproperties.Clear();
        using (IEnumerator<PropertyEntry> enumerator = this.easingFunctionProperty.get_PropertyValue().get_SubProperties().GetEnumerator())
        {
          while (((IEnumerator) enumerator).MoveNext())
            this.subproperties.Add(this.easingFunctionProperty.SceneNodeObjectSet.CreateSceneNodeProperty(this.easingFunctionProperty.Reference.Append(((PropertyReferenceProperty) enumerator.Current).Reference), (AttributeCollection) null));
        }
    ...
    private void OnEasingFunctionSelected(object sender, RoutedEventArgs args)
    {
      ...
      using (SceneEditTransaction editTransaction = viewModel.CreateEditTransaction(StringTable.ChangeEasingFunctionUndoUnit, false))
      {
        if (ownEasingFunction != null)
        {
          ...
          this.easingFunctionProperty.get_PropertyValue().set_Value(ownEasingFunction.PlatformSpecificObject);
        }
        else
          this.easingFunctionProperty.get_PropertyValue().ClearValue();
    ...
    private void UpdateFromDataContext()
    {
      ...
      {
        this.easingFunctionProperty = propertyValue.get_ParentProperty() as SceneNodeProperty;
    ...
    private void Rebuild()
    {
      if (this.knownEasingFunctions.Count == 0 && this.easingFunctionProperty != null)
      {
        ...
        if (viewModel.ProjectContext.ResolveType(PlatformTypes.EasingFunctionBase) != null)
        {
          ...
          this.knownEasingFunctionsView = CollectionViewSource.GetDefaultView(this.knownEasingFunctions);
    ...
    private void UpdateEasingFunctionSelected()
    {
      if (this.easingFunctionProperty != null && this.easingFunctionProperty.SceneNodeObjectSet.Count > 0)
      {
        ...
        {
          object platformObject = this.easingFunctionProperty.get_PropertyValue().get_Value();
    ...
    public void InitializeComponent()
    {
      ...
      Application.LoadComponent(this, new Uri("/Microsoft.Expression.DesignSurface;component/userinterface/propertyinspector/categoryeditors/easing/easingfunctioneditor.xaml", UriKind.Relative));
    }
