...
namespace Microsoft.Expression.DesignSurface.UserInterface.PropertyInspector
{
  ...
  public sealed class LayoutPathCollectionEditor : ComplexValueEditorBase, IPickWhipHost, IComponentConnector, IStyleConnector
  {
    ...
    private PropertyValueCollection LayoutPaths
    {
      get
      {
        if (this.propertyValueCache != null)
          return ((PropertyValue) this.propertyValueCache).get_Collection();
    ...
    private void ChangeOrder(int increment)
    {
      ...
      int num = currentPosition + increment;
      if (currentPosition == -1 || num < 0 || num >= this.LayoutPaths.get_Count())
        return;
      using (SceneEditTransaction editTransaction = this.ViewModel.CreateEditTransaction(StringTable.ChangeLayoutPathOrderUndo))
      {
        ...
        this.LayoutPaths.SetIndex(currentPosition, num);
    ...
    private void DeleteLayoutPath(object sender, RoutedEventArgs args)
    {
      ...
      if (nodePropertyValue == null || this.LayoutPaths == null || this.LayoutPaths.get_Count() == 0)
        return;
      if (this.LayoutPathsView.CurrentItem as SceneNodePropertyValue == nodePropertyValue && this.LayoutPathsView.CurrentPosition == this.LayoutPaths.get_Count() - 1)
        this.currentIndex = Math.Max(0, this.LayoutPaths.get_Count() - 2);
      using (SceneEditTransaction editTransaction = this.ViewModel.CreateEditTransaction(StringTable.RemoveLayoutPathUndo))
      {
        ...
        {
          this.LayoutPaths.Remove((PropertyValue) nodePropertyValue);
          editTransaction.Commit();
        }
      }
    }
    ...
    protected override void Rebuild()
    {
      ...
      this.currentIndex = 0;
      this.LayoutPathsView = CollectionViewSource.GetDefaultView(this.LayoutPaths);
      this.LayoutPathsView.CollectionChanged += new NotifyCollectionChangedEventHandler(this.LayoutPathsView_CollectionChanged);
      this.LayoutPathsView.CurrentChanged += new EventHandler(this.LayoutPathsView_CurrentChanged);
    ...
    {
      if (this.LayoutPathsView == null || this.LayoutPaths == null || (this.LayoutPaths.get_Count() == 0 || this.LayoutPathsView.CurrentPosition == -1))
        return;
    ...
    {
      this.OnPropertyChanged("HasLayoutPaths");
      if (this.LayoutPathsView == null || this.LayoutPaths == null || this.LayoutPaths.get_Count() == 0)
        return;
      if (this.currentIndex >= this.LayoutPaths.get_Count())
        this.currentIndex = this.LayoutPaths.get_Count() - 1;
    ...
    public void InitializeComponent()
    {
      ...
      Application.LoadComponent(this, new Uri("/Microsoft.Expression.DesignSurface;component/userinterface/propertyinspector/complexvalueeditors/layoutpathcollectioneditor.xaml", UriKind.Relative));
    }
