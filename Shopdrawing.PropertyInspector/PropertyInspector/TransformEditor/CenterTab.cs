  ...
  public sealed class CenterTab : Grid, ITransformEditorTab, IComponentConnector
  {
    ...
    public void UpdatePropertyContainers(TransformPropertyLookup propertyLookup)
    {
      ...
      if (reference.Count == 1 && reference.FirstStep.Equals((object) Base2DElement.RenderTransformProperty))
      {
        ...
        {
          this.CenterPointXEditor.set_PropertyEntry((PropertyEntry) null);
          this.CenterPointYEditor.set_PropertyEntry((PropertyEntry) null);
        }
        else
        {
          this.CenterPointXEditor.set_PropertyEntry((PropertyEntry) propertyLookup.CreateNormalProperty(this.renderTransformOriginXProperty, attributes));
          this.CenterPointYEditor.set_PropertyEntry((PropertyEntry) propertyLookup.CreateNormalProperty(this.renderTransformOriginYProperty, attributes));
        }
      }
      else
      {
        this.CenterPointXEditor.set_PropertyEntry((PropertyEntry) propertyLookup.CreateProperty("CenterX", attributes));
        this.CenterPointYEditor.set_PropertyEntry((PropertyEntry) propertyLookup.CreateProperty("CenterY", attributes));
      }
      if (propertyLookup.TransformType == TransformType.Transform3D)
        this.CenterPointZEditor.set_PropertyEntry((PropertyEntry) propertyLookup.CreateProperty("CenterZ", attributes));
      else
        this.CenterPointZEditor.set_PropertyEntry((PropertyEntry) null);
    }
