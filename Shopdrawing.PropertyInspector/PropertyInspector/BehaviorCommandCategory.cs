...
namespace Microsoft.Expression.DesignSurface.UserInterface.PropertyInspector
{
  public sealed class BehaviorCommandCategory : SceneNodeCategory
  {
      ...
      : base(CategoryLocalizationHelper.CategoryName.BehaviorCommand, localizedName, messageLogger)
    {
      ...
      this.TriggerNodeView = CollectionViewSource.GetDefaultView(this.TriggerNodes);
    ...
    internal void AddTriggerNode()
    {
      ...
      using (SceneEditTransaction editTransaction = this.sceneViewModel.CreateEditTransaction(StringTable.AddBehaviorCommandTriggerUndo))
      {
        ...
        commandActionNode.CommandName = this.get_CategoryName();
    ...
    internal void DeleteTriggerNode(SceneNodeProperty triggerProperty)
    {
      ...
      {
        valueAsSceneNode.TargetType.Name
      })))
      {
    ...
    private void ClearTriggerPropertiesList()
    {
      ...
      using (IEnumerator<PropertyEntry> enumerator = this.TriggerNodes.GetEnumerator())
      {
        while (((IEnumerator) enumerator).MoveNext())
          ((PropertyBase) enumerator.Current).OnRemoveFromCategory();
      }
    ...
    private void UpdateProperties()
    {
      ...
      if (triggerProperty != null)
      {
        ...
        this.triggerBasicPropertiesViewSource.Source = this.properties.Properties;
        this.triggerAdvancedPropertiesViewSource.Source = this.properties.Properties;
      }
      else
      {
        this.triggerBasicPropertiesViewSource.Source = null;
        this.triggerAdvancedPropertiesViewSource.Source = null;
      }
      this.UpdateViews();
    }
    ...
    {
      BehaviorCommandCategory.PropertyEntryCollectionWrapper collectionWrapper = new BehaviorCommandCategory.PropertyEntryCollectionWrapper(triggerProperty.get_PropertyValue().get_SubProperties());
      ...
      {
        if (ProjectNeutralTypes.InvokeCommandAction.IsAssignableFrom((ITypeId) triggerActionNode.Type) && ((InvokeCommandActionNode) triggerActionNode).CommandName.Equals(this.get_CategoryName(), StringComparison.Ordinal))
        {
    ...
    private void Rebuild()
    {
      ...
      foreach (SceneNode sceneNode in (IEnumerable<SceneNode>) collectionForProperty)
      {
        ...
        foreach (BehaviorTriggerActionNode triggerActionNode in (IEnumerable<SceneNode>) behaviorTriggerBaseNode.Actions)
        {
          ...
          if (commandActionNode != null && commandActionNode.IsAttached && string.Compare(commandActionNode.CommandName, this.get_CategoryName(), StringComparison.OrdinalIgnoreCase) == 0)
          {
    ...
    private void TriggerBasicPropertiesFilter(object sender, FilterEventArgs e)
    {
      ...
      e.Accepted = !propertyEntry.get_IsAdvanced();
    }
    ...
    private void UpdateViews()
    {
      ...
      if (this.sceneViewModel == null || this.sceneViewModel.DesignerContext == null)
        return;
      base.ApplyFilter(new PropertyFilter(this.sceneViewModel.DesignerContext.PropertyInspectorModel.FilterString));
    }
    ...
    private sealed class PropertyEntryCollectionWrapper
    {
      ...
      public IEnumerable<PropertyReferenceProperty> Properties
      {
        get
        {
          ...
          using (IEnumerator<PropertyEntry> enumerator = this.propertyEntryCollection.GetEnumerator())
          {
            while (((IEnumerator) enumerator).MoveNext())
            {
              PropertyReferenceProperty referenceProperty = enumerator.Current as PropertyReferenceProperty;
