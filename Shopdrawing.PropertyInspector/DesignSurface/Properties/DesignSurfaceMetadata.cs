...
namespace Microsoft.Expression.DesignSurface.Properties
{
  internal class DesignSurfaceMetadata
  {
    ...
    private static DesignSurfaceMetadata.AssemblyInformation CreateInteractionsAssemblyInformation()
    {
      ...
      PropertyOrder default1 = PropertyOrder.get_Default();
      ...
      PropertyOrder default2 = PropertyOrder.get_Default();
      ...
      PropertyOrder before = PropertyOrder.CreateBefore(PropertyOrder.get_Early());
    ...
    private static DesignSurfaceMetadata.AssemblyInformation CreateSketchFlowInteractivityAssemblyInformation()
    {
      ...
      assemblyInformation.AddPropertyAttributes(ProjectNeutralTypes.NavigationMenuAction, "TargetScreen", (Attribute) PropertyValueEditor.CreateEditorAttribute(typeof (NavigableScreenPickerPropertyValueEditor)), (Attribute) new PropertyOrderAttribute(PropertyOrder.get_Early()), (Attribute) new CategoryAttribute(CategoryNames.CategoryCommonProperties), (Attribute) new DescriptionAttribute(InteractivityStringTable.Description_NavigationMenuAction_TargetScreen));
    ...
    private static AttributeTableBuilder BuildAttributeTable(Assembly assembly)
    {
        ...
        DesignSurfaceMetadata.u003cu003ec__DisplayClass19 variable = null;
        DesignSurfaceMetadata.u003cu003ec__DisplayClass16 variable1 = null;
        AttributeTableBuilder attributeTableBuilder;
        DesignSurfaceMetadata.PropertyInformation propertyInformation;
        AttributeTableBuilder attributeTableBuilder1 = null;
        DesignSurfaceMetadata.AssemblyInformation assemblyInformation = null;
        AssemblyName assemblyName = AssemblyHelper.GetAssemblyName(assembly);
        if (assembly != null && DesignSurfaceMetadata.assemblyInformation.TryGetValue(assemblyName.Name, out assemblyInformation) && assemblyInformation.HasTypes)
        {
            types = null;
            try
            {
            ...
            {
                attributeTableBuilder = null;
            }
            return attributeTableBuilder;
        }
        return attributeTableBuilder1;
    Label0:
        DesignSurfaceMetadata.SDKVersion versionFromAssembly = DesignSurfaceMetadata.SDKVersion.GetVersionFromAssembly(assembly);
        attributeTableBuilder1 = new AttributeTableBuilder();
        ...
        for (int i = 0; i < (int)typeArray.Length; i++)
        {
            ...
            if (assemblyInformation.TryGetTypeInformation(type.FullName, out typeInformation))
            {
                ...
                if (typeInformation.GetAttributesForVersion(versionFromAssembly).Count > 0)
                {
                    AttributeTableBuilder attributeTableBuilder2 = attributeTableBuilder1;
                    Type type1 = type;
                    if (attributeCallback == null)
                    {
                        attributeCallback = new AttributeCallback(variable1, (AttributeCallbackBuilder builder) => builder.AddCustomAttributes(this.typeAttributes.ToArray<Attribute>()));
                    }
                    attributeTableBuilder2.AddCallback(type1, attributeCallback);
                }
                if (typeInformation.HasProperties)
                {
                    ...
                    for (int j = 0; j < (int)properties.Length; j++)
                    {
                        ...
                        AttributeCallback attributeCallback1 = null;
                        if (typeInformation.TryGetPropertyInformation(propertyInfo.Name, out propertyInformation))
                        {
                            AttributeTableBuilder attributeTableBuilder3 = attributeTableBuilder1;
                            Type type2 = type;
                            if (attributeCallback1 == null)
                            {
                                attributeCallback1 = new AttributeCallback(variable, (AttributeCallbackBuilder builder) => builder.AddCustomAttributes(this.propertyInfo.PropertyName, this.propertyInfo.GetAttributesForVersion(this.CSu0024u003cu003e8__locals14.assemblyVersion).ToArray<Attribute>()));
                            }
                            attributeTableBuilder3.AddCallback(type2, attributeCallback1);
                        }
                    }
                }
            }
